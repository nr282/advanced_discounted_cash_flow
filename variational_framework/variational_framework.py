...
def correct_function(function):
    test_val = function(0)
    if type(test_val) == np.ndarray:
        function_lambda_corrected = lambda x: float(function(x)[0])
        return function_lambda_corrected
    else:
        return function
...
class VariationalFramework(object):
    ...
    def get_function_integrated(self,
        ...
        if function_id in self.function_id_to_function_lambdas:
            function = self.function_id_to_function_lambdas.get(function_id)
            corrected_function = correct_function(function)
            integral_val, _ = integrate.quad(corrected_function, start_time, end_time)
            return integral_val
        else:
            raise ValueError(f"Function ID {function_id} not found in function_id_to_function_lambdas.")
